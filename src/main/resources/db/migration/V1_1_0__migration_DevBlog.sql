...
create table post (id integer not null auto_increment, is_active tinyint(1) not null, moderation_status enum('NEW','ACCEPTED','DECLINED') default 'NEW' not null, text text not null, time timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(), title varchar(255) not null, view_count int not null, moderator_id int NOT NULL COMMENT 'id пользователя', user_id int NOT NULL COMMENT 'id пользователя' not null, primary key (id)) engine=MyISAM;
